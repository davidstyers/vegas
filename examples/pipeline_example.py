...
from vegas.strategy import Signal, Strategy
...
class PipelineStrategy(Strategy):
    ...
    def initialize(self, context):
        ...
        context.set_commission(
            commission.PerShare(cost_per_share=0.005, min_trade_cost=1.0)
        )
    ...
    def handle_data(self, context, data):
        ...
        try:
            ...
            context.porfolio_postions = (
                {pos.symbol for pos in positions} if positions else set()
            )
        except Exception as e:
        ...
        def get_price(sym: str) -> float | None:
            try:
                s = data.filter(pl.col("symbol") == sym).select("close").to_series()
                if s.len() == 0:
                    return None
                return float(s.item())
            except Exception:
        ...
        for symbol in context.longs:
            if symbol not in context.porfolio_postions and symbol in symbols_in_data:
                ...
                if hasattr(context.portfolio, "current_equity"):
                    position_value = (
                        context.portfolio.current_equity * context.position_size
                    )
                    quantity = int(position_value / price) if price > 0 else 10

                if quantity > 0:
                    signals.append(
                        Signal(symbol=symbol, action="BUY", quantity=quantity)
                    )
    ...
    def on_market_close(self, context, data, portfolio):
        ...
        pct_change = (
            current_portfolio_value - start_portfolio_value
        ) / start_portfolio_value
        print(
            f"Current portfolio value: {context.portfolio.get_portfolio_value():.2f} / {pct_change:.2%}"
        )
...
def make_pipeline():
    ...
    volume_sma = SimpleMovingAverage(inputs=["volume"], window_length=30)

    price_sma = SimpleMovingAverage(inputs=["close"], window_length=10)

    liquid_stocks = volume_sma > 100_000  # Only stocks with decent volume
    no_pennies = price_sma > 2.00

    return Pipeline(
        columns={"momentum": momentum, "volatility": volatility},
