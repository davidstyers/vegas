...
def connect_to_database(db_path="db/vegas.duckdb"):
    ...
    if not os.path.exists(db_path):
        ...
        print(
            "Please ingest data first using: vegas ingest-ohlcv --directory path/to/data"
        )
        return None
...
def analyze_market_data(conn, output_dir="results"):
    ...
    if not volume_df.empty:
        ...
        top_symbols = (
            volume_df.groupby("symbol")["daily_volume"]
            .sum()
        ...
        # Pivot for plotting
        pivot_df = top_volume_df.pivot(
            index="date", columns="symbol", values="daily_volume"
        )
    ...
    if not volatility_df.empty:
        ...
        volatile_symbols = (
            volatility_df.groupby("symbol")["daily_volatility"]
            .mean()
            .sort_values(ascending=False)
            .head(5)
            .index.tolist()
        ...
        # Pivot for plotting
        pivot_df = vol_df.pivot(
            index="date", columns="symbol", values="daily_volatility"
        )
...
def main():
    ...
    parser.add_argument(
        "--db-path", default="db/vegas.duckdb", help="Path to DuckDB database"
    )
    parser.add_argument(
        "--output-dir", default="results", help="Directory to save visualizations"
    )
