...
def mock_data_portal():
    ...
    class MockDataPortal:
        ...
        def history(self, assets=None, fields=None, bar_count=1, frequency='1d', end_dt=None):
...
def test_run_pipeline_with_factors(mock_data_portal):
    ...
    pipeline = Pipeline(columns={'sma_3': SimpleMovingAverage(window_length=3)}, frequency='1d')
...
def test_run_pipeline_with_screen(mock_data_portal):
    ...
    pipeline = Pipeline(columns={'sma_3': SimpleMovingAverage(window_length=3)}, screen=StaticAssets(['AAPL']), frequency='1d')
