...
from vegas.broker.ibkr_adapter import IBKRConfig, InteractiveBrokersBrokerAdapter
...
def test_ibkr_offline_place_and_cancel():
    broker = InteractiveBrokersBrokerAdapter(
        IBKRConfig(host="127.0.0.1", port=7497, client_id=1)
    )
    ...
    assert all(
        str(o.get("id")) != str(oid) or o.get("status") != "open" for o in open_orders2
    )


def test_ibkr_offline_account_positions_seed_and_get():
    broker = InteractiveBrokersBrokerAdapter()
    broker._seed_account(
        cash=100000.0, positions={"MSFT": {"quantity": 5, "avg_cost": 300.0}}
    )
...
def test_ibkr_field_mapping_variants():
    ...
    assert t == "MKT" and f == {}
    # Explicit Market
    t, f = brk._resolve_order_fields(
        Signal(symbol="AAPL", quantity=1, order_type="market")
    )
    assert t == "MKT"
    # Limit
    t, f = brk._resolve_order_fields(
        Signal(symbol="AAPL", quantity=1, order_type="limit", limit_price=123.45)
    )
    assert t == "LMT" and "lmtPrice" in f
    # Stop
    t, f = brk._resolve_order_fields(
        Signal(symbol="AAPL", quantity=1, stop_price=120.0)
    )
    assert t == "STP" and f.get("auxPrice") == 120.0
    # Stop Limit
    t, f = brk._resolve_order_fields(
        Signal(symbol="AAPL", quantity=1, stop_price=120.0, limit_price=121.0)
    )
...
@pytest.mark.skipif(
    os.environ.get("HAS_IBAPI", "0") != "1", reason="ibapi not available in test env"
)
