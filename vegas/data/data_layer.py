...
class DataLayer:
    ...
    def __init__(
        self, data_dir: str = "db", test_mode: bool = False, timezone: str = "EST"
    ):
    ...
    def __del__(self):
        ...
        if (
            ...
            and self.parquet_dir.startswith(tempfile.gettempdir())
        ):
            try:
                ...
                if os.path.exists(self.parquet_dir):
                    ...
                    self.logger.info(
                        f"Cleaned up temporary test directory: {self.parquet_dir}"
                    )
            except Exception as e:
                self.logger.warning(
    ...
    def is_initialized(self) -> bool:
        ...
        if self.use_database and self.db_manager:
            try:
                ...
                return (
                    not dates.is_empty()
                    and dates.row(0)[dates.columns.index("day_count")] > 0
                )
            except Exception as e:
    ...
    def _try_load_from_database(self) -> None:
        ...
        if self.use_database and self.db_manager:
            try:
                ...
                if (
                    not dates.is_empty()
                    and dates.select(pl.col("day_count")).item() > 0
                ):
    ...
    def _load_single_file(self, file_path: str) -> None:
        ...
        if (
            file_path.endswith(".ohlcv-1h.csv.zst")
    ...
    def _load_multiple_files(
        self, directory: str = None, file_pattern: str = "*.csv*", max_files: int = None
    ) -> None:
        ...
        self.logger.info(
            f"Loading data from directory: {directory} with pattern: {file_pattern}"
        )

        # Handle OHLCV directory for database ingestion
        if (
            file_pattern == "*.ohlcv-1h.csv.zst"
            and self.use_database
            and self.db_manager
        ):
            try:
                self.db_manager.ingest_ohlcv_directory(
        ...
        if not files:
            raise FileNotFoundError(
                f"No data files found in {directory} with pattern {file_pattern}"
            )
    ...
    def _validate_and_process_data(self, df: pl.DataFrame) -> None:
        ...
        self.date_range = (
            df.select("timestamp").min().item(),
            df.select("timestamp").max().item(),
        )

        self.logger.info(f"Processed {len(df)} rows with {len(self.symbols)} symbols")

    def get_unified_timestamp_index(
        ...
        if not self.use_database or self.db_manager is None:
            ...
            if self.data is None or self.data.is_empty():
                return pl.Series(
                    "timestamp", [], dtype=pl.Datetime(time_zone=self.timezone)
                )
            ...
            try:
                filtered = self.data.filter(
                    (
                        pl.col("timestamp")
                        >= pl.lit(start_bound).cast(pl.Datetime("us", self.timezone))
                    )
                    & (
                        pl.col("timestamp")
                        <= pl.lit(end_bound).cast(pl.Datetime("us", self.timezone))
                    )
                )
            except Exception:
            ...
            ts = (
                filtered.select(pl.col("timestamp"))
                .unique()
                .sort("timestamp")
        ...
        try:
            ts = self.db_manager.get_unified_timestamps(
                start_ts, end_ts, timezone=self.timezone
            )
            return ts
        except Exception as e:
    ...
    def get_universe(self, date: datetime.date = None) -> List[str]:
        ...
        if self.data is not None and not self.data.is_empty():
            try:
                ...
                date_floor, date_ceil = (
                    date_floor.astimezone(pytz.timezone(self.timezone)),
                    date_ceil.astimezone(pytz.timezone(self.timezone)),
                )

                filtered_data = self.data.filter(
                    (
                        pl.col("timestamp")
                        >= pl.lit(date_floor).cast(pl.Datetime("us", self.timezone))
                    )
                    & (
                        pl.col("timestamp")
                        <= pl.lit(date_ceil).cast(pl.Datetime("us", self.timezone))
                    )
                )
                if not filtered_data.is_empty():
                    symbols = (
                        filtered_data.select("symbol").unique().to_series().to_list()
                    )
                    return symbols
                else:
                    ...
                    if not nearby_data.is_empty():
                        ...
                        symbols = (
                            nearby_data.select("symbol").unique().to_series().to_list()
                        )
                        return symbols
            except Exception as e:
    ...
    def get_available_date_range(self) -> tuple:
        ...
        if self.use_database and self.db_manager:
            try:
                ...
                if not dates.is_empty():
                    out = (
                        dates.select("start_date").item(),
                        dates.select("end_date").item(),
                    )
                    return out
            except Exception:
                pass
        ...
        if self.data is not None and not self.data.is_empty():
            out = (
                self.data.select(pl.col("timestamp").min()).item(),
                self.data.select(pl.col("timestamp").max()).item(),
            )
            return out
    ...
    def get_data_info(self) -> Dict:
        ...
        if start_date is None or end_date is None:
            return {"symbol_count": 0, "start_date": None, "end_date": None, "days": 0}
    ...
    def ingest_ohlcv_directory(self, directory: str, max_files: int = None) -> int:
        ...
        try:
            rows_affected = self.db_manager.ingest_ohlcv_directory(
                directory=directory, pattern="*.ohlcv-1h.csv.zst", max_files=max_files
            )
    ...
    def get_data_for_timestamp(
        ...
        df = self.get_data_for_backtest(
            start=timestamp, end=timestamp, symbols=sym_list, market_hours=market_hours
        )
    ...
    def get_trading_days(self, start_date: datetime, end_date: datetime) -> pl.Series:
        ...
        filtered = self.data.filter(
            (
                pl.col("timestamp")
                >= pl.lit(start_ts).cast(pl.Datetime("us", self.timezone))
            )
            & (
                pl.col("timestamp")
                <= pl.lit(end_ts).cast(pl.Datetime("us", self.timezone))
            )
        )
        ...
        dates_df = (
            filtered.select(pl.col("timestamp").dt.date().alias("date"))
            .unique()
            .sort("date")
        )
    ...
    def history(
        ...
        if frequency != native_frequency:
            self.logger.info(
                f"Resampling data from '{native_frequency}' to '{frequency}'"
            )
            ...
            if agg_exprs:
                ...
                self.logger.warning(
                    f"Resampling for fields {fields} is not defined; returning un-resampled data."
                )
        ...
        if not df.is_empty() and symbols:
            try:
                ...
                if pivot_values:
                    df = df.pivot(
                        index="timestamp", columns="symbol", values=pivot_values
                    )
            except Exception as e:
                self.logger.warning(
                    f"Could not pivot data to wide format: {e}. Returning long format."
                )
