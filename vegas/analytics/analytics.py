...
try:
    ...
    HAS_QS = False
    logging.warning("QuantStats not installed. Some analytics features will be unavailable.")
...
class Results:
    ...
    def create_tearsheet(self, title: str = "Strategy Performance", 
                        benchmark_symbol: str = None, output_file: str = None,
        ...
        if output_file:
            if output_format.lower() == 'html':
                if benchmark_returns is not None:
                    qs.reports.html(returns_series, benchmark_returns, 
                                 title=title, output=output_file)
                elif benchmark_symbol:
                    qs.reports.html(returns_series, benchmark_symbol,
                                 title=title, output=output_file)
                else:
                ...
                if benchmark_returns is not None:
                    qs.reports.full(returns_series, benchmark_returns, 
                                  title=title, output=output_file)
                elif benchmark_symbol:
                    qs.reports.full(returns_series, benchmark_symbol,
                                  title=title, output=output_file)
                else:
                ...
                logging.error(f"Unsupported output format: {output_format}. Use 'html' or 'pdf'.")
        else:
            # Display in notebook
            if benchmark_returns is not None:
    ...
    def export_results(self, format_type: str, file_path: str) -> None:
        ...
        if format_type.lower() == 'csv':
            ...
            raise ValueError(f"Unsupported export format: {format_type}. Use 'csv', 'json', 'html', or 'tearsheet'.")
...
class Analytics:
    ...
    @staticmethod
    def calculate_stats(portfolio, broker, start_date, end_date, benchmark_data=None) -> Dict[str, Any]:
        ...
        if not returns.empty and HAS_QS:
            ...
            stats = {
                ...
                'total_return_pct': ((portfolio.current_equity / portfolio.initial_cash) - 1) * 100,
                'cagr': qs.stats.cagr(returns_series) * 100 if len(returns_series) > 1 else 0,
                'sharpe_ratio': qs.stats.sharpe(returns_series) if len(returns_series) > 1 else 0,
                'sortino_ratio': qs.stats.sortino(returns_series) if len(returns_series) > 1 else 0,
                'max_drawdown': qs.stats.max_drawdown(returns_series) * 100 if len(returns_series) > 1 else 0,
                'volatility_annualized': qs.stats.volatility(returns_series) * 100 if len(returns_series) > 1 else 0,
            ...
            if len(returns_series) > 1:
                try:
                    stats.update({
                        ...
                        'cvar': qs.stats.cvar(returns_series) * 100,  # Conditional VaR
                        'best_day': qs.stats.best(returns_series) * 100,
                        'worst_day': qs.stats.worst(returns_series) * 100,
                        'avg_up_month': qs.stats.avg_win(returns_series, aggregate='M') * 100,
                        'avg_down_month': qs.stats.avg_loss(returns_series, aggregate='M') * 100
                    })
        ...
        if benchmark_data is not None and HAS_QS and not returns.empty:
            ...
            try:
                stats.update({
                    'alpha': qs.stats.alpha(returns_series, benchmark_returns) * 100,
                    'beta': qs.stats.beta(returns_series, benchmark_returns),
                    'correlation': qs.stats.correlation(returns_series, benchmark_returns),
                    'r_squared': qs.stats.r_squared(returns_series, benchmark_returns),
                    'information_ratio': qs.stats.information_ratio(returns_series, benchmark_returns),
                    'treynor_ratio': qs.stats.treynor_ratio(returns_series, benchmark_returns),
                })
            except Exception as e:
